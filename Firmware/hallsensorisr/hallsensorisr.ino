...
void IRAM_ATTR isr() {
  hall_time = millis();
  if (hall_time - hall1.lastdebounceTime >= hall1.debounceDelay){ //debouce logic (only looking at input that happens every X millis())   
    bool edge = digitalRead(hall1.PIN); // true = rising, false = falling 

      report = true;
      hall1.numberKeyPresses++;
      hall1.pressed = true;
      hall1.lastdebounceTime = hall_time;
...
void setup() {
  ...
  attachInterrupt(hall1.PIN, isr, RISING);
